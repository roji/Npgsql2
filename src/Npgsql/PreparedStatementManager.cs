...
namespace Npgsql
{
    class PreparedStatementManager
    {
        ...
        internal PreparedStatement? GetOrAddExplicit(string sql, List<NpgsqlParameter> parameters)
        {
            ...
            if (BySql.TryGetValue(sql, out var pStatement))
            {
                ...
                if (pStatement.IsExplicit)
                {
                    ...
                    return pStatement.DoParametersMatch(parameters)
                        ? pStatement
                        : null;
                }
            ...
            return BySql[sql] = PreparedStatement.CreateExplicit(this, sql, NextPreparedStatementName(), parameters, statementBeingReplaced);
        }

        internal PreparedStatement? TryGetAutoPrepared(string sql, List<NpgsqlParameter> parameters)
        {
            if (!BySql.TryGetValue(sql, out var pStatement))
            {
            ...
            case PreparedState.BeingPrepared:
                ...
                // We just need to check that the parameter types correspond, since prepared statements are
                // only keyed by SQL (to prevent pointless allocations). If we have a mismatch, simply run unprepared.
                if (!pStatement.DoParametersMatch(parameters))
                    return null;
